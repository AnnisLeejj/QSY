...
public class ComplaintAdapter extends BaseAdapter {
	...
	public ComplaintAdapter(Context context,
			ArrayList<ComplaintReportingBean.Data> ComplaintReportinglist,
...
//				layou.mState.setTextColor(Color.parseColor("#026598"));
		...
		case 11:
			...
			if (list.get(porINt % list.size()).getComplainant() == null) {
				layou.mContext.setText("匿名的投诉或者举报被核实或者是采纳"
						+(TextUtils.isEmpty(list.get(porINt % list.size())
								.getTheComplainantReward())?"":","+list.get(porINt % list.size())
										.getTheComplainantReward()));
			} else {
				layou.mContext.setText(list.get(porINt % list.size())
						.getComplainant()
						+ "的投诉或者举报被核实或者是采纳"
						+ (TextUtils.isEmpty(list.get(porINt % list.size())
								.getTheComplainantReward())?"":","+list.get(porINt % list.size())

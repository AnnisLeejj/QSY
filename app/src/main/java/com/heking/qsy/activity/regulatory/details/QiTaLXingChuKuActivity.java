 ...
 * @author jhw
 */
public class QiTaLXingChuKuActivity extends Activity implements OnRefreshListener {
    ...
    private QiTaLXingChuKuAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_qitaleixingchuku.data> list = new ArrayList<TB_qitaleixingchuku.data>();
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_qitaleixingchuku TB_qitaleixingchuku = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_qitaleixingchuku.class);
                list = TB_qitaleixingchuku.getData();
                adapter = new QiTaLXingChuKuAdapter(QiTaLXingChuKuActivity.this, list, data);
            ...
            public void onClick(View arg0) {
                ...
                HttpHelper.getInstance().service.get(url.trim()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_qitaleixingchuku TB_qitaleixingchuku = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_qitaleixingchuku.class);
                        for (TB_qitaleixingchuku.data datas : TB_qitaleixingchuku.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_other_outStock", data.getFirmID(), 1, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getKuCun(data.getFirmID());
            HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                @Override
                public void onSubscribe(Disposable d) {

...
public class YLJGActivity extends BaseActivity {
    ...
    private ArrayList<FirmTypeBean.Data> firmlistChild = new ArrayList<FirmTypeBean.Data>();
    ...
    private void listData() {
        if (FirmTapeDataList != null) {
            for (FirmTypeBean.Data data : FirmTapeDataList) {
                if (data.getFirmTypeName1().equals("医疗机构")) {
                    if (data.getMonitors() != null && data.getMonitors().size() > 0) {

                        data.setIoos(true);

                    } else {
                        firmlistChild.add(data);
                        data.setIoos(false);
                    }

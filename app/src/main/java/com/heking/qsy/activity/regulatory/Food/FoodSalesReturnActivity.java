...
import com.heking.qsy.activity.regulatory.Food.adapter.FoodSalesReturnAdapter;
 ...
 * @author jhw
 */
public class FoodSalesReturnActivity extends Activity implements OnRefreshListener {
    private String url;
    ...
    private FoodSalesReturnAdapter adapter;
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                Log.d("message", json);
                TB_FoodSalsesReturn TB_FoodSalsesReturn = ParsonJson.jsonToBeanObj("{\"Data\":" + json + "}", TB_FoodSalsesReturn.class);
                if (TB_FoodSalsesReturn != null) {
                    list = TB_FoodSalsesReturn.getData();
                    adapter = new FoodSalesReturnAdapter(FoodSalesReturnActivity.this, list, data, uTil);

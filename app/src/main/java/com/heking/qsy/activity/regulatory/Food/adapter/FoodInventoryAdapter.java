...
public class FoodInventoryAdapter extends BaseAdapter {
	...
	public  FoodInventoryAdapter(Context context,ArrayList<TB_FoodInventory.Data> list,FirmTypeBean.Data  data,RegulatoryUTil uTil){
		this.list=list;
	...
	public View getView(final int pronts, View arg1, ViewGroup arg2) {
		...
		case 2:
			...
			layout.Nmaetype.setText(new Tooluite().getDate(list.get(pronts).getProductionDate(),"T")+list.get(pronts).getSupplier());
			break;
		case 4:
			layout.Name.setText(list.get(pronts).getGenericName());
			layout.Nmaetype.setText(new Tooluite().getDate(list.get(pronts).getProductionDate(),"T")+list.get(pronts).getSupplier());
		...
		case 5:
			layout.Name.setText(list.get(pronts).getGenericName());
			layout.Nmaetype.setText(new Tooluite().getDate(list.get(pronts).getUploadTime(),"T"));
		default:
			break;
		}

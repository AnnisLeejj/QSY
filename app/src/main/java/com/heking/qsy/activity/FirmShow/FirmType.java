...
import com.heking.qsy.activity.ConvenienceService.util.DHPoriMap;
	...
	private void iniData() {
		...
		if (data.getMonitors() != null) {
			if (data.getMonitors().size() > 0) {
				...
				if (AppContext.shipins != null) {
					for (Data.Monitors item : data.getMonitors()) {
						for (com.hikvision.vmsnetsdk.CameraInfo item1 : AppContext.shipins) {
				...
				MonitorsAdapter adapter = new MonitorsAdapter(cameraInfos, this);

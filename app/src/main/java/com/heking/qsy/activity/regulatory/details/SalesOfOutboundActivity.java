...
import com.heking.qsy.activity.regulatory.details.adapter.SalesOfOutboundAdapter;
 ...
 * @author jhw
 */
public class SalesOfOutboundActivity extends Activity implements OnRefreshListener {
    ...
    private SalesOfOutboundAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_XiaoShuoTuiHuo.data> list = new ArrayList<TB_XiaoShuoTuiHuo.data>();
    private int page = 1;
    ...
    private void iniView() {
        ...
        findViewById(R.id.view_show_totototoo).setVisibility(TextView.GONE);
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_XiaoShuoTuiHuo TB_XiaoShuoTuiHuo = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_XiaoShuoTuiHuo.class);
                list = TB_XiaoShuoTuiHuo.getData();
                adapter = new SalesOfOutboundAdapter(SalesOfOutboundActivity.this, list);
        ...
        mButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                dialog.show();
                String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_sellReturnStock", data.getFirmID(), 1, 20, mes);

                //			    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getToURl("get_firm_data_sellReturnStock",data.getFirmID());
                list.clear();
                ...
                HttpHelper.getInstance().service.get(url.trim()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_XiaoShuoTuiHuo TB_XiaoShuoTuiHuo = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_XiaoShuoTuiHuo.class);
                        for (TB_XiaoShuoTuiHuo.data datas : TB_XiaoShuoTuiHuo.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_sellReturnStock", data.getFirmID(), page, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getToURl("get_firm_data_sellReturnStock",data.getFirmID());

...
import com.heking.qsy.activity.regulatory.details.adapter.QiTaLeiXingRuKuAdapter;
 ...
 * @author jhw
 */
public class QiTaLeiXingRuKuActivity extends Activity implements OnRefreshListener {
    ...
    private QiTaLeiXingRuKuAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_QiTaLeiXinRuKu.data> list = new ArrayList<TB_QiTaLeiXinRuKu.data>();
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_QiTaLeiXinRuKu TB_QiTaLeiXinRuKu = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_QiTaLeiXinRuKu.class);
                list = TB_QiTaLeiXinRuKu.getData();
                adapter = new QiTaLeiXingRuKuAdapter(QiTaLeiXingRuKuActivity.this, list);
        ...
        mButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                dialog.show();
                String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_other_inStock", data.getFirmID(), 1, 20, "");
                ...
                HttpHelper.getInstance().service.get(url.trim()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_QiTaLeiXinRuKu TB_QiTaLeiXinRuKu = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_QiTaLeiXinRuKu.class);
                        for (TB_QiTaLeiXinRuKu.data datas : TB_QiTaLeiXinRuKu.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_other_inStock", data.getFirmID(), page, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getKuCun(data.getFirmID());

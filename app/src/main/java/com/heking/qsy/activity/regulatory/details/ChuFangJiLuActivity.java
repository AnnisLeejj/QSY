...
import com.heking.qsy.activity.regulatory.details.adapter.ChuFangJiLuAdapter;
 ...
 * @author jhw
 */
public class ChuFangJiLuActivity extends Activity implements OnRefreshListener {
    ...
    private ChuFangJiLuAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_Chunfangjilu.data> list = new ArrayList<TB_Chunfangjilu.data>();
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_Chunfangjilu TB_Chunfangjilu = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_Chunfangjilu.class);
                list = TB_Chunfangjilu.getData();
                adapter = new ChuFangJiLuAdapter(ChuFangJiLuActivity.this, list);
        ...
        mButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                dialog.show();
                String url = ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA) + ToolUtil.getToURl("get_firm_data_PrescribeRecords", data.getFirmID(), page, 20, mes);

                //			    String	url=ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA)+ToolUtil.getToURl("get_firm_data_PrescribeRecords",data.getFirmID());
                list.clear();
                HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_Chunfangjilu TB_Chunfangjilu = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_Chunfangjilu.class);
                        for (TB_Chunfangjilu.data datas : TB_Chunfangjilu.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA) + ToolUtil.getToURl("get_firm_data_PrescribeRecords", data.getFirmID(), page, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA)+ToolUtil.getToURl("get_firm_data_PrescribeRecords",data.getFirmID());
            HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                @Override
                public void onSubscribe(Disposable d) {

...
import com.heking.qsy.activity.regulatory.Food.Bevera.adapter.FoodBeveraTryToSampleAdapter;
 ...
 * @author jhw
 */
public class FoodBeveraTryToSampleActivity extends Activity implements OnRefreshListener {
    private String url;
    ...
    private FoodBeveraTryToSampleAdapter adapter;
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                Log.d("message", json);
                TB_FoodBeveraTryToSample TB_FoodBeveraTryToSample = ParsonJson.jsonToBeanObj("{\"Data\":" + json + "}", TB_FoodBeveraTryToSample.class);
                if (TB_FoodBeveraTryToSample != null) {
                    list = TB_FoodBeveraTryToSample.getData();
                    adapter = new FoodBeveraTryToSampleAdapter(FoodBeveraTryToSampleActivity.this, list, data);

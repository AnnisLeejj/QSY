...
import com.heking.qsy.activity.regulatory.Food.Bevera.adapter.FoodBeveraRawMaterialInventoryReturnAdapter;
 ...
 * @author jhw
 */
public class FoodBeveraRawMaterialInventoryReturnActivity extends Activity implements OnRefreshListener {
    private String url;
    ...
    private ArrayList<TB_BeveraRawMaterialInventory.Data> list;

    private FoodBeveraRawMaterialInventoryReturnAdapter adapter;
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                dialog.dismiss();
                Log.d("message", json);
                TB_BeveraRawMaterialInventory TB_BeveraRawMaterialInventory = ParsonJson.jsonToBeanObj("{\"Data\":" + json + "}", TB_BeveraRawMaterialInventory.class);
                if (TB_BeveraRawMaterialInventory != null) {
                    list = TB_BeveraRawMaterialInventory.getData();
                    adapter = new FoodBeveraRawMaterialInventoryReturnAdapter(FoodBeveraRawMaterialInventoryReturnActivity.this, list, data);

...
public class ProcurementWarehousingAdapter extends BaseAdapter {
	private ArrayList<TB_CaiGouRuKu.data>  list;
	private Context context;
	private LayoutInflater mInflater;
	private int state;
	private FirmTypeBean.Data data;
	public  ProcurementWarehousingAdapter(Context context,ArrayList<TB_CaiGouRuKu.data> list,FirmTypeBean.Data data){
		this.list=list;
	...
	public View getView(final int pronts, View arg1, ViewGroup arg2) {
		...
		layout.cose();
		layout.Name.setText(list.get(pronts).getPurchasedID());
		...
		layout.Nmaetype.setText(med+"-"+list.get(pronts).getSupplier());
		layout.button.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				String url=ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA)+ToolUtil.getType("Get_procure_in_Storages_detail_list", list.get(pronts).getID());
				Intent intent=new Intent();
				Bundle bundle=new Bundle();
				
				bundle.putSerializable("data", data);
				bundle.putSerializable("MESSAGE", list.get(pronts));
				bundle.putInt("statess", 1310031);

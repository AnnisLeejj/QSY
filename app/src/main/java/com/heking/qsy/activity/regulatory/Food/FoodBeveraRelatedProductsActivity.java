...
import com.heking.qsy.activity.regulatory.Food.Bevera.adapter.FoodBeveraRelatedProductsAdapter;
 ...
 * @author jhw
 */
public class FoodBeveraRelatedProductsActivity extends Activity implements OnRefreshListener {
    private String url;
    ...
    private ArrayList<TB_BeveraProcurement.Data> list;

    private FoodBeveraRelatedProductsAdapter adapter;
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                Log.d("message", json);
                TB_BeveraProcurement TB_BeveraProcurement = ParsonJson.jsonToBeanObj("{\"Data\":" + json + "}", TB_BeveraProcurement.class);
                if (TB_BeveraProcurement != null) {
                    list = TB_BeveraProcurement.getData();
                    adapter = new FoodBeveraRelatedProductsAdapter(FoodBeveraRelatedProductsActivity.this, list, data);

...
import com.heking.qsy.activity.regulatory.details.adapter.TeShuYaoPinXiaoShouAdapter;
 ...
 * @author jhw
 */
public class TeShuYaoPinXiaoShouActivity extends Activity implements OnRefreshListener {
    ...
    private TeShuYaoPinXiaoShouAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_TeShuYaoPinXiaoShou.data> list = new ArrayList<TB_TeShuYaoPinXiaoShou.data>();
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_TeShuYaoPinXiaoShou TB_TeShuYaoPinXiaoShou = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_TeShuYaoPinXiaoShou.class);
                list = TB_TeShuYaoPinXiaoShou.getData();
                adapter = new TeShuYaoPinXiaoShouAdapter(TeShuYaoPinXiaoShouActivity.this, list);
        ...
        mButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                dialog.show();
                String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_special_sellOutStock", data.getFirmID(), 1, 20, "");
                ...
                HttpHelper.getInstance().service.get(url.trim()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_TeShuYaoPinXiaoShou TB_TeShuYaoPinXiaoShou = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_TeShuYaoPinXiaoShou.class);
                        for (TB_TeShuYaoPinXiaoShou.data datas : TB_TeShuYaoPinXiaoShou.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_special_sellOutStock", data.getFirmID(), page, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getKuCun(data.getFirmID());
            HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                @Override
                public void onSubscribe(Disposable d) {

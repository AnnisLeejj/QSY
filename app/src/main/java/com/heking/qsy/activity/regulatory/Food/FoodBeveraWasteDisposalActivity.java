...
import com.heking.qsy.activity.regulatory.Food.Bevera.adapter.FoodBeveraWasteDisposalAdapter;
 ...
 * @author jhw
 */
public class FoodBeveraWasteDisposalActivity extends Activity implements OnRefreshListener {
    private String url;
    ...
    private ArrayList<TB_FoodBeveraWasteDisposal.Data> list;

    private FoodBeveraWasteDisposalAdapter adapter;
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                Log.d("message", json);
                TB_FoodBeveraWasteDisposal TB_FoodBeveraWasteDisposal = ParsonJson.jsonToBeanObj("{\"Data\":" + json + "}", TB_FoodBeveraWasteDisposal.class);
                if (TB_FoodBeveraWasteDisposal != null) {
                    list = TB_FoodBeveraWasteDisposal.getData();
                    adapter = new FoodBeveraWasteDisposalAdapter(FoodBeveraWasteDisposalActivity.this, list, data);

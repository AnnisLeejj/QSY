...
public class PurchaseReturnAdapter extends BaseAdapter {
	...
	private ArrayList<TB_CaiGouTuiHuo.data>  list;
	private Context context;
	private LayoutInflater mInflater;
	private int state;
	private FirmTypeBean.Data data;
	public  PurchaseReturnAdapter(Context context,ArrayList<TB_CaiGouTuiHuo.data> list){
		this.list=list;
	...
	public View getView(final int pronts, View arg1, ViewGroup arg2) {
		...
		layout.cose();
		layout.Name.setText(list.get(pronts).getReturnNumber());
		...
		layout.button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				String url=ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA)+ToolUtil.getType("Get_procure_return_Storages_detail_list", list.get(pronts).getID());
				Intent intent=new Intent();
				Bundle bundle=new Bundle();
				
				bundle.putSerializable("data", data);
				bundle.putSerializable("MESSAGE", list.get(pronts));
				bundle.putInt("statess", 1310032);

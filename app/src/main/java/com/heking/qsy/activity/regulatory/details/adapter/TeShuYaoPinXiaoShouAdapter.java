...
public class TeShuYaoPinXiaoShouAdapter extends BaseAdapter {
	
	private ArrayList<TB_TeShuYaoPinXiaoShou.data>  list;
	private Context context;
	private LayoutInflater mInflater;
	private int state;
	private FirmTypeBean.Data data;
	public  TeShuYaoPinXiaoShouAdapter(Context context,ArrayList<TB_TeShuYaoPinXiaoShou.data> list){
		this.list=list;
	...
	public View getView(final int pronts, View arg1, ViewGroup arg2) {
		...
		layout.cose();
		layout.Name.setText(list.get(pronts).getSaleOrderNumber());
		layout.Nmaetype.setText(new Tooluite().getDate(list.get(pronts).getSaleTime())+"-" +list.get(pronts).getPurchaseMan());
		layout.button.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				String url=ToolUtil.DataUrl.getUrl(AppContext.Parameter.GET_FIRM_DATA)+ToolUtil.getType("Get_Special_Sell_Out_Storages_detail_list", list.get(pronts).getID());
				Intent intent=new Intent();
				Bundle bundle=new Bundle();
				
				bundle.putSerializable("data", data);
				bundle.putSerializable("MESSAGE", list.get(pronts));
				bundle.putInt("statess", 1310036);

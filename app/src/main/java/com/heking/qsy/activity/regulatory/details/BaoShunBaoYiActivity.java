...
import com.heking.qsy.activity.regulatory.details.adapter.BaoShunBaoYiAdapter;
 ...
 * @author jhw
 */
public class BaoShunBaoYiActivity extends Activity implements  OnRefreshListener {
    ...
    private BaoShunBaoYiAdapter adapter;
    private RefreshListView listView;
    private ArrayList<TB_BaoShunBaoYin.data> list = new ArrayList<TB_BaoShunBaoYin.data>();
    private int page = 1;
    ...
    private void iniData() {
        ...
        HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
            ...
            public void onSuccess(String json) {
                ...
                TB_BaoShunBaoYin TB_BaoShunBaoYin = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_BaoShunBaoYin.class);
                list = TB_BaoShunBaoYin.getData();
                adapter = new BaoShunBaoYiAdapter(BaoShunBaoYiActivity.this, list);
        ...
        mButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                dialog.show();
                String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_decreaseOverflows", data.getFirmID(), page, 20, mes);

                //			    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getToURl("get_firm_data_decreaseOverflows",data.getFirmID());
                list.clear();
                ...
                HttpHelper.getInstance().service.get(url).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<String>() {
                    ...
                    public void onSuccess(String json) {
                        JsongDataBean bean = ParsonJson.jsonToBeanObj(json, JsongDataBean.class);
                        TB_BaoShunBaoYin TB_BaoShunBaoYin = ParsonJson.jsonToBeanObj("{\"data\":" + bean.getJsonData() + "}", TB_BaoShunBaoYin.class);
                        for (TB_BaoShunBaoYin.data datas : TB_BaoShunBaoYin.getData()) {
                            list.add(datas);
                        }
                        dialog.dismiss();
    ...
    public void onLoadingMore() {
        if (boo) {
            page++;
            String url = ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android") + ToolUtil.getToURl("get_firm_data_decreaseOverflows", data.getFirmID(), page, 20, "");
//	    String	url=ToolUtil.DataUrl.getUrl("http://117.173.38.55:84/Android")+ToolUtil.getToURl("get_firm_data_decreaseOverflows",data.getFirmID());

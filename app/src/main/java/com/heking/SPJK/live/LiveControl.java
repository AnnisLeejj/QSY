...
import com.heking.SPJK.utils.UtilSDCard;
...
public class LiveControl implements RtspClientCallback, PlayerDisplayCB {
    ...
    private LiveCallBack mLiveActivityCallBack = null;
    ...
    public void setLiveCallBack(LiveCallBack liveCallBack) {
        mLiveActivityCallBack = liveCallBack;
    }
    ...
    private void startRtsp() {
        ...
        if (!ret) {
            ...
            if (null != mLiveActivityCallBack) {
                mLiveActivityCallBack.onMessageCallback(ConstantLive.RTSP_FAIL);
                LogUtils.w("shipin", "摄像头不可用 212", mRtspEngineIndex, mUrl, mDeviceUserName, mDevicePassword);
            }
            return;
        }
        mLiveState = LIVE_STREAM;
        if (null != mLiveActivityCallBack) {
            mLiveActivityCallBack.onMessageCallback(ConstantLive.RTSP_SUCCESS);
        }
    }
    ...
    public void stop() {
        ...
        closePlayer();
        if (null != mLiveActivityCallBack) {
            mLiveActivityCallBack.onMessageCallback(ConstantLive.STOP_SUCCESS);
        }
    ...
    public void onDataCallBack(int handle, int dataType, byte[] data, int length, int timeStamp, int packetNo, int useId) {
        ...
        switch (dataType) {
            case RtspClient.DATATYPE_HEADER:
                ...
                if (!ret) {
                    if (null != mLiveActivityCallBack) {
                        mLiveActivityCallBack.onMessageCallback(ConstantLive.START_OPEN_FAILED);
                        return;
                    } else {
                        DebugLog.error(TAG, "onDataCallBack():: mLiveActivityCallBack is null");
                    }
                } else {
    ...
    public boolean capture(String filePath, String picName) {
        if (!UtilSDCard.isSDCardUsable()) {
            if (null != mLiveActivityCallBack) {
                mLiveActivityCallBack.onMessageCallback(ConstantLive.SD_CARD_UN_USEABLE);
            }
            return false;
        }
        ...
        if (UtilSDCard.getSDCardRemainSize() <= mSDCardSize) {
            if (null != mLiveActivityCallBack) {
                mLiveActivityCallBack.onMessageCallback(ConstantLive.SD_CARD_SIZE_NOT_ENOUGH);
            }
            return false;
        }

        if (LIVE_PLAY != mLiveState) {
            mLiveActivityCallBack.onMessageCallback(ConstantLive.CAPTURE_FAILED_NPLAY_STATE);
            return false;
        }
    ...
    public boolean startRecord(String filePath, String fileName) {
        ...
        if (LIVE_PLAY != mLiveState) {
            ...
            mLiveActivityCallBack.onMessageCallback(ConstantLive.RECORD_FAILED_NPLAY_STATE);
            return false;
        }
    ...
    public boolean startAudio() {
        if (LIVE_PLAY != mLiveState) {
            ...
            mLiveActivityCallBack.onMessageCallback(ConstantLive.AUDIO_START_FAILED_NPLAY_STATE);
            return false;
        }
    ...
    public Calendar getOSDTime() {
        ...
        if (!ret) {
            ...
            mLiveActivityCallBack.onMessageCallback(ConstantLive.GET_OSD_TIME_FAIL);
            return systemTime;
        }
    ...
    public void onDisplay(int arg0, byte[] arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7) {
        if (LIVE_PLAY != mLiveState) {
            ...
            if (null != mLiveActivityCallBack) {
                mLiveActivityCallBack.onMessageCallback(ConstantLive.PLAY_DISPLAY_SUCCESS);
            } else {
                DebugLog.error(TAG, "onDisplay():: mLiveActivityCallBack is null");
            }
